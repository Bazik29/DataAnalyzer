...
class Presenter():
    ...
    def _view_loadX(self):
        ...
        source = "../" + \
            self._model.savefig(
                self._model.infoX['graph'], self._model.title + "-histX", "plots/")
    ...
    def _view_loadXY(self):
        ...
        source = "../" + \
            self._model.savefig(
                self._model.infoXY['graph'], self._model.title + "-scatter", "plots/")
        self._view.pch_setgraphsource(source)
    ...
    def _view_loadRegress(self):
        ...
        source = "../" + \
            self._model.savefig(
                self._model.infoReg['graph'], self._model.title + "-regress", "plots/")
        self._view.preg_setgraphsource(source)

        urav = "y = {:.2f}*x{:+.2f}".format(
            self._model.infoReg['coef'], self._model.infoReg['intercept'])
        self._view.preg_insertvalues(urav, round(self._model.infoReg['R'], 4), round(self._model.infoReg['R2'], 4), round(
            self._model.infoReg['std'], 4), round(self._model.infoReg['coef'], 4), self._model.infoReg['count'])
    ...
    def _view_loadDisper(self):
        ...
        self._view.pdis_insertvalues(
            round(self._model.infoDisp['f'], 4), round(self._model.infoDisp['p'], 4))
    ...
    def _view_reportClick(self):
        ...
        if elements['graphX']:
            ...
            source = self._model.savefig(
                self._model.infoX['graph'], self._model.title + "-histX", "plots/")
            src = self._model.encodePNG(source)
            img = self._model.genImg("Гистограмма величины: " + self._model.nameX, src)
            content.append(img)
        ...
        if elements['crits']:
            ...
            table = self._model.genTableCrits(
                "Нормальность распределения величины: " + self._model.nameX, D1, pvl1, D2, pvl2)
            content.append(table)
